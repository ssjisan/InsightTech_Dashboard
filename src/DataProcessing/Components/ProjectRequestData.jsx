...
export default function ProjectRequestData() {
    ...
    useEffect(() => {
        ...
        axios.get('https://insighttechbd-2c31b-default-rtdb.firebaseio.com/ClientRequest.json')
            .then(response => {
    ...
    const handleRemoveRequest = (id) => {
        ...
        axios.delete(`https://insighttechbd-2c31b-default-rtdb.firebaseio.com/ClientRequest/${id}.json`)
            .then(() => {
                console.log('Request removed successfully');
                // Update clientRequests state after successful deletion
                setAllRequests(prevState => prevState.filter(request => request.id !== id));
            })
            .catch(error => {
